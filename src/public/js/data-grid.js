$(document).ready(function(){   
    ...
    $('.grid input[type="checkbox"].grid-checkbox').click(function(){
        if($(this).is(':checked') && $(this).closest('table').find('tbody input[type="checkbox"].grid-checkbox').not(':checked').length===0){
            $(this).closest('table').find('input[type="checkbox"].selecionar-todos').prop('checked', true);
        }else{
            $(this).closest('table').find('input[type="checkbox"].selecionar-todos').prop('checked', false);
        }
    });

    $('.grid-container .acoes-em-massa select').change(function(){
        if($(this).val()){
    ...
    $('.grid-container select[name=exportar]').change(function(){
        if($(this).val()){
            $(this).closest('.grid-container').find('.bt-exportar').attr('href', $(this).closest('.grid-container').find('.bt-exportar').attr('data-href')+'&exportar='+$(this).val() );
        }else{
            $(this).closest('.grid-container').find('.bt-exportar').attr('href', '#');         
        }
    });

    $('.grid-container .bt-exportar').click(function(e){
        if($(this).attr('href')=='#'){
            e.preventDefault();
